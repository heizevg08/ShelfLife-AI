...
export function ingredientControllers(service: IngredientService): Record<'list' | 'create' | 'update' | 'remove', RequestHandler> {
  return {
    ...
    update: async (req, res) => { const ingredient = await service.update(res.locals.user.id, objectId(req.params.id), ingredientInput(req.body)); if (!ingredient) { res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Ingredient not found', details: [] } }); return; } res.json({ ingredient }); },
    remove: async (req, res) => { const removed = await service.remove(res.locals.user.id, objectId(req.params.id)); if (!removed) { res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Ingredient not found', details: [] } }); return; } res.status(204).end(); },
  };
}
